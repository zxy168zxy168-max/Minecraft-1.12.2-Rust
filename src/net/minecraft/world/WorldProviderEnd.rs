...
impl WorldProviderEnd {
    ...
    pub const fn getDimensionType() -> DimensionType { DimensionType::TheEnd }
    pub const fn calculateCelestialAngle() -> f32 { 0.0 }
    ...
    pub const fn isSkyColored() -> bool { false }
    pub const fn canRespawnHere() -> bool { false }
    pub const fn isSurfaceWorld() -> bool { false }
    pub const fn getCloudHeight() -> f32 { 8.0 }
    pub const fn getSpawnCoordinate() -> BlockPos { BlockPos::new(100,50,0) }
    pub const fn getAverageGroundLevel() -> i32 { 50 }
    pub const fn doesXZShowFog() -> bool { false }
    /// Unlike the Nether, MCP WorldProviderEnd never sets `hasNoSky=true`.
    pub const fn hasNoSky() -> bool { false }
}
