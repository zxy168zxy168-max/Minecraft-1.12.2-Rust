...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TempCategory { Cold, Medium, Warm }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BiomeClass {
    Ocean, Plains, Desert, Hills, Forest, Taiga, Swamp, River, Hell, End, Snow,
    MushroomIsland, Beach, Jungle, StoneBeach, Savanna, Mesa, Void,
    ForestMutated, SavannaMutated,
}
...
impl Biome {
    ...
    pub const fn getBaseHeight(self) -> f32 { match self.id {
        0|10 => -1.0, 1|2|12|35|129 => 0.125, 3|34|131|162 => 1.0,
        5|14|30|32|149|151|155|157|160|161 => 0.2, 6 => -0.2, 7|11 => -0.5,
        15|16|26 => 0.0, 17|18|19|22|28|31|33 => 0.45, 20 => 0.8,
        24 => -1.8, 36|38|39 => 1.5, 130 => 0.225, 133|158 => 0.3,
        134 => -0.1, 140 => 0.425, 156 => 0.55, 163 => 0.3625, 164 => 1.05,
        166|167 => 0.45, _ => 0.1,
    }}

    pub const fn getHeightVariation(self) -> f32 { match self.id {
        0|6|10|24 => 0.1, 1|2|12|35|129 => 0.05, 3|34|131|162 => 0.5,
        7|11 => 0.0, 13|17|18|19|20|22|28|31|33|166|167 => 0.3,
        14 => 0.3, 15|16|26|36|38|39 => 0.025, 25 => 0.8,
        130 => 0.25, 132|133|149|151|155|157|158 => 0.4,
        134 => 0.3, 140 => 0.45000002, 156 => 0.5, 163 => 1.225,
        164 => 1.2125001, _ => 0.2,
    }}

    pub const fn getTemperature(self) -> f32 { self.temperature }
    pub const fn isSnowyBiome(self) -> bool { matches!(self.id,10|11|12|13|26|30|31|140|158) }
    pub const fn isMutation(self) -> bool { matches!(self.id,129|130|131|132|133|134|140|149|151|155|156|157|158|160|161|162|163|164|165|166|167) }
    pub fn getMutationForBiome(self) -> Option<Self> { Some(Self::getBiome(match self.id {
        1=>129,2=>130,3=>131,4=>132,5=>133,6=>134,12=>140,21=>149,23=>151,
        27=>155,28=>156,29=>157,30=>158,32=>160,33=>161,34=>162,35=>163,
        36=>164,37=>165,38=>166,39=>167,_=>return None,
    })) }
    pub const fn getBiomeClass(self) -> BiomeClass { match self.id {
    ...
    pub const fn getTempCategory(self) -> TempCategory {
        if self.temperature < 0.2 { TempCategory::Cold }
        else if self.temperature < 1.0 { TempCategory::Medium }
        else { TempCategory::Warm }
    }
    pub const fn isMesa(self) -> bool { matches!(self.getBiomeClass(),BiomeClass::Mesa) }
    pub const fn isJungleClass(self) -> bool { matches!(self.getBiomeClass(),BiomeClass::Jungle) }

    pub const fn getRainfall(self) -> f32 { self.rainfall }
    pub const fn getWaterColor(self) -> i32 { self.waterColor }
    pub const fn ignorePlayerSpawnSuitability(self) -> bool { self.id == 127 }
    ...
    pub fn generateBiomeTerrain(self, sea_level:i32, rand:&mut JavaRandom, primer:&mut ChunkPrimer, x:i32, z:i32, noise:f64, top_in:IBlockState, filler_in:IBlockState) {
        ...
        for y in (0..=255_i32).rev(){
            ...
            if remaining==-1 {
                if thickness<=0 { top=AIR; filler=STONE; }
                else if y>=sea_level-4 && y<=sea_level+1 { top=top_in; filler=filler_in; }
                if y<sea_level && top.isAir(){
                    top=if self.getFloatTemperature(BlockPos::new(x,y,z))<0.15 {ICE}else{WATER};
                }
                remaining=thickness;
                if y>=sea_level-1 {
                    primer.setBlockState(local_x,y as usize,local_z,top);
                } else if y<sea_level-7-thickness {
                    top=AIR; filler=STONE;
                ...
                if remaining==0 && filler.getBlockId()==12 && thickness>1 {
                    ...
                    filler=if filler.getMetadata()==1{RED_SANDSTONE}else{SANDSTONE};
                }
            }
        }
    }
