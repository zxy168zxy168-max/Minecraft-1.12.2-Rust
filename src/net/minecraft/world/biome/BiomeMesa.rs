...
impl BiomeMesa {
    pub fn forBiome(biome: Biome) -> Self {
        Self {
            clayBands: None,
            worldSeed: 0,
            pillarNoise: None,
            pillarRoofNoise: None,
            clayBandsOffsetNoise: None,
            brycePillars: biome.getId() == 165,
            hasForest: matches!(biome.getId(), 38 | 166),
        }
    }
    ...
    fn generateBands(&mut self, seed: i64) {
        ...
        while l1 < 64 {
            ...
            if l1 < 64 {
                bands[l1 as usize] = ORANGE;
            }
            l1 += 1; // Java for-loop post increment after body
        }
        let i2 = random.next_i32_bound(4) + 2;
        for _ in 0..i2 {
            let width = random.next_i32_bound(3) + 1;
            let start = random.next_i32_bound(64);
            for off in 0..width {
                if start + off < 64 {
                    bands[(start + off) as usize] = IBlockState::fromGlobalStateId(STAINED | 4);
                }
            }
        }
        let j2 = random.next_i32_bound(4) + 2;
        for _ in 0..j2 {
            let width = random.next_i32_bound(3) + 2;
            let start = random.next_i32_bound(64);
            for off in 0..width {
                if start + off < 64 {
                    bands[(start + off) as usize] = IBlockState::fromGlobalStateId(STAINED | 12);
                }
            }
        }
        ...
        for _ in 0..l2 {
            ...
            for off in 0..width {
                if start + off < 64 {
                    bands[(start + off) as usize] = IBlockState::fromGlobalStateId(STAINED | 14);
                }
            }
        }
        ...
        for _ in 0..k3 {
            ...
            if pos < 64 {
                ...
                if pos > 1 && random.next_bool() {
                    bands[(pos - 1) as usize] = IBlockState::fromGlobalStateId(STAINED | 8);
                }
                if pos < 63 && random.next_bool() {
                    bands[(pos + 1) as usize] = IBlockState::fromGlobalStateId(STAINED | 8);
                }
            }
        }
    ...
    fn getBand(&self, x: i32, y: i32, _z: i32) -> IBlockState {
        let noise = self
            ...
            .as_ref()
            .expect("mesa clay offset initialized")
            .getValue(x as f64 / 512.0, x as f64 / 512.0)
            * 2.0;
    ...
    pub fn genTerrainBlocks(
        ...
        if self.clayBands.is_none() || self.worldSeed != worldSeed {
            self.generateBands(worldSeed);
        }
        if self.pillarNoise.is_none()
            || self.pillarRoofNoise.is_none()
        ...
        if self.brycePillars {
            let i = (x & -16) + (z & 15);
            let j = (z & -16) + (x & 15);
            let d0 = noiseVal.abs().min(
                self.pillarNoise
                    .as_ref()
                    .unwrap()
                    .getValue(i as f64 * 0.25, j as f64 * 0.25),
            );
            if d0 > 0.0 {
                let d2 = self
                    .pillarRoofNoise
                    .as_ref()
                    .unwrap()
                    .getValue(i as f64 * 0.001953125, j as f64 * 0.001953125)
                    .abs();
                pillar_height = d0 * d0 * 2.5;
                let cap = (d2 * 50.0).ceil() + 14.0;
                if pillar_height > cap {
                    pillar_height = cap;
                }
                pillar_height += 64.0;
            }
        ...
        let mut current = STAINED;
        let mut filler_state = filler;
        ...
        let mut orange_cap = false;
        let mut stone_count = 0_i32;
        for y in (0..=255_i32).rev() {
            if primer.getBlockState(local_x, y as usize, local_z).isAir()
                && y < (pillar_height as i32)
            {
                primer.setBlockState(local_x, y as usize, local_z, STONE);
            }
            if y <= rand.next_i32_bound(5) {
                primer.setBlockState(local_x, y as usize, local_z, BEDROCK);
                continue;
            }
            if stone_count >= 15 && !self.brycePillars {
                continue;
            }
            ...
            if remaining == -1 {
                ...
                if y < seaLevel && current.isAir() {
                    current = WATER;
                }
                remaining = thickness + (y - seaLevel).max(0);
                if y >= seaLevel - 1 {
                    if self.hasForest && y > 86 + thickness * 2 {
                        primer.setBlockState(
                            local_x,
                            y as usize,
                            local_z,
                            if flag { COARSE_DIRT } else { GRASS },
                        );
                    } else if y > seaLevel + 3 + thickness {
                        let surface = if (64..=127).contains(&y) {
                            if flag {
                                HARD
                            } else {
                                self.getBand(x, y, z)
                    ...
                    if filler_state.getBlockId() == 159 {
                        primer.setBlockState(local_x, y as usize, local_z, ORANGE);
                    }
                }
            } else if remaining > 0 {
                remaining -= 1;
