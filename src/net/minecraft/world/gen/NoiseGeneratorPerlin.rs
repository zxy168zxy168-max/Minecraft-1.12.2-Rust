...
impl NoiseGeneratorPerlin {
    ...
    pub fn getRegion(&self, reuse: Option<Vec<f64>>, x_start:f64, z_start:f64, x_size:i32, z_size:i32, x_scale:f64, z_scale:f64, persistence:f64)->Vec<f64>{
        self.getRegionWithLacunarity(reuse,x_start,z_start,x_size,z_size,x_scale,z_scale,persistence,0.5)
    }

    /// MCP `NoiseGeneratorPerlin#getRegion(..., persistence, lacunarity)`.
    #[allow(clippy::too_many_arguments)]
    pub fn getRegionWithLacunarity(&self, reuse:Option<Vec<f64>>, x_start:f64, z_start:f64, x_size:i32, z_size:i32, x_scale:f64, z_scale:f64, persistence:f64, lacunarity:f64)->Vec<f64>{
        let len=(x_size as usize).saturating_mul(z_size as usize);
        let mut out=match reuse{
            Some(mut array) if array.len()>=len=>{array.fill(0.0);array},
            _=>vec![0.0;len],
        };
        let mut d1=1.0_f64;
        let mut d0=1.0_f64;
        for level in 0..self.levels{
            self.noiseLevels[level].add(&mut out,x_start,z_start,x_size,z_size,x_scale*d0*d1,z_scale*d0*d1,0.55/d1);
            d0*=persistence;
            d1*=lacunarity;
        }
        out
...
mod tests {
    ...
    fn region_matches_mcp_java_golden_values(){
        ...
        let expected=[3.4828151133767897,3.4818697607465197,3.5042085698579055,3.4576300501007770,3.4452382825008880,3.4634651153733493,3.4348122788824350,3.4117051982336690,3.4254189294270083,3.4127431564036095,3.3796681503495978,3.3884563995903760];
        assert_eq!(actual.len(),expected.len());
        for(a,e)in actual.iter().zip(expected){assert!((*a-e).abs()<1.0e-14,"{a} != {e}");}
    }
}
