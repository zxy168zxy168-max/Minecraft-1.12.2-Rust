...
#[derive(Debug)]
pub struct GenLayerAddIsland {
    seed: GenLayerSeed,
    parent: Layer,
}
impl GenLayerAddIsland {
...
impl GenLayer for GenLayerAddIsland {
    ...
    fn getInts(&mut self, x: i32, z: i32, w: i32, h: i32) -> Vec<i32> {
        ...
        for yy in 0..h {
            for xx in 0..w {
                let nw = p[(xx + (yy) * pw) as usize];
                let ne = p[(xx + 2 + yy * pw) as usize];
                let sw = p[(xx + (yy + 2) * pw) as usize];
                let se = p[(xx + 2 + (yy + 2) * pw) as usize];
                let c = p[(xx + 1 + (yy + 1) * pw) as usize];
                self.seed.initChunkSeed((xx + x) as i64, (yy + z) as i64);
                let v = if c != 0 || (nw == 0 && ne == 0 && sw == 0 && se == 0) {
                    if c > 0 && (nw == 0 || ne == 0 || sw == 0 || se == 0) {
                        if self.seed.nextInt(5) == 0 {
                            if c == 4 {
                                4
                            } else {
                                0
                    ...
                    let mut count = 1;
                    let mut pick = 1;
                    for n in [nw, ne, sw, se] {
                        if n != 0 {
                            if self.seed.nextInt(count) == 0 {
                                pick = n;
                            }
                            count += 1;
                        }
