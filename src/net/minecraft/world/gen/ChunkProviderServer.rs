...
impl std::fmt::Debug for ChunkProviderServer {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ChunkProviderServer")
...
impl ChunkProviderServer {
    ...
    pub fn withGenerator(
        chunkLoaderIn: AnvilChunkLoader,
        chunkGeneratorIn: Box<dyn IChunkGenerator>,
    ) -> Self {
        Self {
            droppedChunksSet: HashSet::new(),
            chunkLoader: chunkLoaderIn,
    ...
    pub fn getLoadedChunks(&self) -> impl Iterator<Item = &Chunk> {
        self.id2ChunkMap.values()
    }
    pub fn getLoadedChunkRef(&self, x: i32, z: i32) -> Option<&Chunk> {
        self.id2ChunkMap.get(&ChunkPos::asLong(x, z))
    }
    pub fn getLoadedChunksMut(&mut self) -> impl Iterator<Item = &mut Chunk> {
        self.id2ChunkMap.values_mut()
    }
    ...
    pub fn loadChunk(&mut self, world: &mut WorldServer, x: i32, z: i32) -> bool {
        ...
        let Some(mut chunk) = loaded else {
            return false;
        };
    ...
    pub fn provideChunk(
        ...
        x: i32,
        z: i32,
    ) -> Result<&mut Chunk, String> {
        let key = ChunkPos::asLong(x, z);
        if !self.id2ChunkMap.contains_key(&key) {
            let loaded = world
                .loadChunkFromLoader(&self.chunkLoader, x, z)
        ...
        let chunk = self
            .id2ChunkMap
            .get_mut(&key)
            .expect("provided chunk inserted");
        chunk.unloaded = false;
        Ok(chunk)
    }
    ...
    pub fn generatorSeaLevelOverride(&self) -> Option<i32> {
        self.chunkGenerator
            .as_ref()
            .and_then(|generator| generator.seaLevelOverride())
    }
    ...
    pub fn unloadAllChunks(&mut self, world: &WorldServer) {
        let coords: Vec<_> = self
            .id2ChunkMap
            .values()
            .map(|c| (c.xPosition, c.zPosition))
            .collect();
        for (x, z) in coords {
            self.unload(world, x, z);
        }
    }
    ...
    pub fn saveChunks(&mut self, world: &mut WorldServer, force: bool) -> bool {
        ...
        for key in keys {
            let Some(chunk) = self.id2ChunkMap.get_mut(&key) else {
                continue;
            };
            if force {
    ...
    pub fn saveExtraData(&self) {
        self.chunkLoader.saveExtraData();
    }
    ...
    pub fn unloadQueuedChunks(&mut self, world: &mut WorldServer) -> bool {
        ...
        for key in queued {
            ...
            let shouldUnload = self
                .id2ChunkMap
                .get(&key)
                .is_some_and(|chunk| chunk.unloaded);
            if !shouldUnload {
                continue;
            }
    ...
    pub fn canSave(&self, world: &WorldServer) -> bool {
        !world.disableLevelSaving
    }

    pub fn makeString(&self) -> String {
        format!(
            "ServerChunkCache: {} Drop: {}",
            self.id2ChunkMap.len(),
            self.droppedChunksSet.len()
        )
    }

    pub fn getLoadedChunkCount(&self) -> usize {
        self.id2ChunkMap.len()
    }
    pub fn chunkExists(&self, x: i32, z: i32) -> bool {
        self.id2ChunkMap.contains_key(&ChunkPos::asLong(x, z))
    }
    ...
    pub fn chunkLoader(&self) -> &AnvilChunkLoader {
        &self.chunkLoader
    }
}
...
mod tests {
    ...
    fn source_cache_and_spawn_unload_contract_are_preserved() {
        ...
        let loader = handler
            .getChunkLoader(&crate::net::minecraft::world::WorldProvider::WorldProvider::new(0))
            .unwrap();
        let mut provider = ChunkProviderServer::new(loader);
        let world = WorldServer::new(handler, info, 0);
        provider
            .id2ChunkMap
            .insert(ChunkPos::asLong(0, 0), Chunk::new(0, 0));
        provider.unload(&world, 0, 0);
        assert!(
            !provider
                .id2ChunkMap
                .get(&ChunkPos::asLong(0, 0))
                .unwrap()
                .unloaded
        );
        provider
            .id2ChunkMap
            .insert(ChunkPos::asLong(30, 30), Chunk::new(30, 30));
        provider.unload(&world, 30, 30);
        assert!(
            provider
                .id2ChunkMap
                .get(&ChunkPos::asLong(30, 30))
                .unwrap()
                .unloaded
        );
        let _ = std::fs::remove_dir_all(root);
