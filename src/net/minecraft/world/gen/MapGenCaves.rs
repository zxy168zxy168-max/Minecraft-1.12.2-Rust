...
impl MapGenCaves {
    ...
    pub fn generate(
        ...
        for source_x in target_x - range..=target_x + range {
            for source_z in target_z - range..=target_z + range {
                let j1 = (source_x as i64).wrapping_mul(j);
                let k1 = (source_z as i64).wrapping_mul(k);
                self.base.rand.set_seed(j1 ^ k1 ^ world_seed);
                self.recursiveGenerate(
                    provider,
                    current_top,
                    source_x,
                    source_z,
    ...
    fn addRoom(
        ...
        let size = 1.0 + self.base.rand.next_f32() * 6.0;
        self.addTunnel(
            provider,
            current_top,
            seed,
    ...
    fn addTunnel(
        ...
        let branch = random.next_i32_bound(max_steps / 2) + max_steps / 4;
        let gentle = random.next_i32_bound(6) == 0;
        while step < max_steps {
            let radius = 1.5 + sin(step as f32 * PI / max_steps as f32) as f64 * size as f64;
            let vertical_radius = radius * vertical_scale;
            let cp = cos(pitch);
            let sp = sin(pitch);
            x += (cos(yaw) * cp) as f64;
            y += sp as f64;
            ...
            if room || random.next_i32_bound(4) != 0 {
                ...
                let max = (size + 2.0 + 16.0) as f64;
                if dx * dx + dz * dz - remain * remain > max * max {
                    return;
                }
                    ...
                    && z <= center_z + 16.0 + radius * 2.0
                {
                    ...
                    let mut min_y = floor_f64(y - vertical_radius) - 1;
                    let mut max_y = floor_f64(y + vertical_radius) + 1;
                    ...
                    max_z = max_z.min(16);
                    let mut water = false;
                    let mut scan_x = min_x;
                    while !water && scan_x < max_x {
                        let mut scan_z = min_z;
                        while !water && scan_z < max_z {
                            let mut scan_y = max_y + 1;
                            while !water && scan_y >= min_y - 1 {
                                if (0..256).contains(&scan_y) {
                                    let id = primer
                                        .getBlockState(
                                            scan_x as usize,
                                            scan_y as usize,
                    ...
                    if !water {
                        for carve_x in min_x..max_x {
                            let ndx = ((carve_x + target_x * 16) as f64 + 0.5 - x) / radius;
                            for carve_z in min_z..max_z {
                                let ndz = ((carve_z + target_z * 16) as f64 + 0.5 - z) / radius;
                                let mut surface = false;
                                if ndx * ndx + ndz * ndz < 1.0 {
                                    for carve_y in ((min_y + 1)..=max_y).rev() {
                                        let ndy =
                                            ((carve_y - 1) as f64 + 0.5 - y) / vertical_radius;
                                        if ndy > -0.7 && ndx * ndx + ndy * ndy + ndz * ndz < 1.0 {
                                            ...
                                            let above = if carve_y < 255 {
                                                primer.getBlockState(
                                                    carve_x as usize,
                                                    (carve_y + 1) as usize,
                                                    carve_z as usize,
                                                )
                                            ...
                                            if state.getBlockId() == 2 || state.getBlockId() == 110
                                            {
                                                surface = true;
                                            }
                                            if Self::canReplaceBlock(state, above) {
                                                if carve_y - 1 < 10 {
                                                    primer.setBlockState(
                                                        carve_x as usize,
                                                        carve_y as usize,
                                                        carve_z as usize,
                                                        IBlockState::fromGlobalStateId(11 << 4),
                                                    );
                                                } else {
                                                    primer.setBlockState(
                                                        carve_x as usize,
    ...
    fn canReplaceBlock(state: IBlockState, above: IBlockState) -> bool {
        match state.getBlockId() {
            1 | 2 | 3 | 24 | 78 | 110 | 159 | 172 | 179 => true,
            12 | 13 => !matches!(above.getBlockId(), 8 | 9),
            _ => false,
        }
    }
    fn recursiveGenerate(
        ...
        let a = self.base.rand.next_i32_bound(15) + 1;
        let b = self.base.rand.next_i32_bound(a) + 1;
        let mut count = self.base.rand.next_i32_bound(b);
        if self.base.rand.next_i32_bound(7) != 0 {
            count = 0;
        }
        for _ in 0..count {
            let x = (source_x * 16 + self.base.rand.next_i32_bound(16)) as f64;
            let inner = self.base.rand.next_i32_bound(120) + 8;
            let y = self.base.rand.next_i32_bound(inner) as f64;
            let z = (source_z * 16 + self.base.rand.next_i32_bound(16)) as f64;
            let mut tunnels = 1;
            if self.base.rand.next_i32_bound(4) == 0 {
                ...
                tunnels += self.base.rand.next_i32_bound(4);
            }
            for _ in 0..tunnels {
                let yaw = self.base.rand.next_f32() * (PI * 2.0);
                let pitch = (self.base.rand.next_f32() - 0.5) * 2.0 / 8.0;
                let mut size = self.base.rand.next_f32() * 2.0 + self.base.rand.next_f32();
                if self.base.rand.next_i32_bound(10) == 0 {
                    size *= self.base.rand.next_f32() * self.base.rand.next_f32() * 3.0 + 1.0;
                }
                let seed = self.base.rand.next_i64();
                self.addTunnel(
                    provider,
