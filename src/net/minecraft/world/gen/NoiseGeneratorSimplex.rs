...
impl NoiseGeneratorSimplex {
    ...
    pub fn add(&self, output: &mut [f64], x_start: f64, z_start: f64, x_size: i32, z_size: i32, x_scale: f64, z_scale: f64, amplitude: f64) {
        ...
        for z_index in 0..z_size {
            ...
            for x_index in 0..x_size {
                ...
                let k2=(self.p[l1+j1 as usize+self.p[i2+k1 as usize] as usize]%12) as usize;
                let l2=(self.p[l1+1+self.p[i2+1] as usize]%12) as usize;
                let contribution=|mut t:f64,g:[i32;3],px:f64,pz:f64|->f64{
                    if t<0.0{0.0}else{t*=t;t*t*Self::dot(g,px,pz)}
                };
