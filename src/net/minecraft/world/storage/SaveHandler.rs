...
impl SaveHandler {
    pub fn new(
        ...
        if storePlayerdata {
            fs::create_dir_all(&playersDirectory)?;
        }
    ...
    pub fn loadWorldInfo(&self) -> io::Result<Option<WorldInfo>> {
        for name in ["level.dat", "level.dat_old"] {
            ...
            let fixed = self
                .dataFixer
                .process(FixTypes::Level, root.getCompoundTag("Data"));
            return Ok(Some(WorldInfo::fromNBT(&fixed)));
        }
        Ok(None)
    }

    pub fn saveWorldInfo(&self, info: &WorldInfo) -> io::Result<()> {
        self.saveWorldInfoWithPlayer(info, None)
    }
    ...
    pub fn saveWorldInfoWithPlayer(
        &self,
        info: &WorldInfo,
        player: Option<&NBTTagCompound>,
    ) -> io::Result<()> {
        ...
        if levelFile.exists() {
            fs::rename(&levelFile, &oldFile)?;
        }
        if levelFile.exists() {
            fs::remove_file(&levelFile)?;
        }
        fs::rename(&newFile, &levelFile)?;
        if newFile.exists() {
            fs::remove_file(newFile)?;
        }
        Ok(())
    }
    ...
    pub fn writePlayerData(&self, player: &EntityPlayerMP) -> io::Result<()> {
        ...
        let uuid = player.getGameProfile().getId().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "player profile has no UUID")
        })?;
    ...
    pub fn readPlayerData(
        &self,
        player: &mut EntityPlayerMP,
    ) -> io::Result<Option<NBTTagCompound>> {
        let uuid = player.getGameProfile().getId().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "player profile has no UUID")
        })?;
        let path = self.playersDirectory.join(format!("{uuid}.dat"));
        if !path.is_file() {
            return Ok(None);
        }
        let raw = CompressedStreamTools::readCompressed(File::open(path)?)?;
        let fixed = self.dataFixer.process(FixTypes::Player, raw);
        player.readFromNBT(fixed.clone());
        Ok(Some(fixed))
    }

    pub fn getWorldDirectory(&self) -> &Path {
        &self.worldDirectory
    }
    pub fn getPlayersDirectory(&self) -> &Path {
        &self.playersDirectory
    }
    pub fn getMapDataDir(&self) -> &Path {
        &self.mapDataDir
    }
}
...
mod player_data_tests {
    ...
    fn playerdata_round_trip_preserves_position_inventory_and_unknown_tags() {
        ...
        let mut original =
            crate::net::minecraft::entity::player::EntityPlayerMP::EntityPlayerMP::new(
                &mut world,
                profile.clone(),
                GameType::Creative,
            )
        ...
        original.inventory.mainInventory[4] = ItemStack {
            itemId: 57,
            count: 23,
            itemDamage: 0,
            tagCompound: None,
        };
        let mut opaque = NBTTagCompound::new();
        opaque.setString("FutureOwner", "preserve-me");
        original.readFromNBT({
            let mut tag = original.writeToNBT();
            tag.setCompoundTag("OpaqueFuture", opaque);
            tag
        });
        world
            .saveHandler()
            .base()
            .writePlayerData(&original)
            .unwrap();
        ...
        let fixed = world
            .saveHandler()
            .base()
            .readPlayerData(&mut loaded)
            .unwrap()
            .unwrap();
        assert_eq!(
            (loaded.entity.posX, loaded.entity.posY, loaded.entity.posZ),
            (12.5, 70.0, -8.25)
        );
        assert_eq!(
            (loaded.entity.rotationYaw, loaded.entity.rotationPitch),
            (33.0, -12.0)
        );
        ...
        assert_eq!(
            fixed
                .getCompoundTag("OpaqueFuture")
                .getString("FutureOwner"),
            "preserve-me"
        );
        assert_eq!(
            loaded
