...
impl WorldSummary {
    pub fn new(info: &WorldInfo, fileName: impl Into<String>, displayName: impl Into<String>, sizeOnDisk: i64, requiresConversion: bool) -> Self {
        Self {
    ...
    pub const fn requiresConversion(&self) -> bool { self.requiresConversion }
    pub const fn getLastTimePlayed(&self) -> i64 { self.lastTimePlayed }
    pub const fn getEnumGameType(&self) -> GameType { self.theEnumGameType }
    pub const fn isHardcoreModeEnabled(&self) -> bool { self.hardcore }
    pub const fn getCheatsEnabled(&self) -> bool { self.cheatsEnabled }
    pub fn getVersionName(&self) -> &str { if self.versionName.is_empty() { "unknown" } else { &self.versionName } }
    pub const fn markVersionInList(&self) -> bool { self.versionId != 1343 || self.versionSnapshot }
    pub const fn askToOpenWorld(&self) -> bool { self.versionId > 1343 }
}

impl Ord for WorldSummary {
    fn cmp(&self, other: &Self) -> Ordering {
        other.lastTimePlayed.cmp(&self.lastTimePlayed).then_with(|| self.fileName.cmp(&other.fileName))
    }
}
