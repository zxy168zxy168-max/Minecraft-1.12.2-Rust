...
impl ChunkPrimer {
    ...
    const fn getBlockIndex(x: usize, y: usize, z: usize) -> usize {
        x << 12 | z << 8 | y
    }
    pub fn getBlockState(&self, x: usize, y: usize, z: usize) -> IBlockState {
        if x >= 16 || z >= 16 || y >= 256 {
            return IBlockState::fromGlobalStateId(0);
        }
    ...
    pub fn setBlockState(&mut self, x: usize, y: usize, z: usize, state: IBlockState) {
        if x < 16 && z < 16 && y < 256 {
            self.data[Self::getBlockIndex(x, y, z)] = state.getGlobalStateId();
        }
    }
    pub fn findGroundBlockIdx(&self, x: usize, z: usize) -> i32 {
        ...
        for y in (0..256).rev() {
            if !self.getBlockState(x, y, z).isAir() {
                return y as i32;
            }
        }
        0
