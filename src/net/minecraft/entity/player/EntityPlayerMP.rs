...
impl EntityPlayerMP {
    ...
    pub fn new(
        ...
        if world.provider.hasSkyLight() && gameType != GameType::Adventure {
            ...
            blockpos = world.getTopSolidOrLiquidBlock(BlockPos::new(
                spawn.x.wrapping_add(dx),
                spawn.y,
                spawn.z.wrapping_add(dz),
            ))?;
        }
        entity.stepHeight = 1.0;
        entity.setPositionAndRotation(
            blockpos.x as f64 + 0.5,
            blockpos.y as f64,
            blockpos.z as f64 + 0.5,
            0.0,
            0.0,
        );
    ...
    pub fn readFromNBT(&mut self, compound: NBTTagCompound) {
        let pos = compound.getTagList("Pos", 6);
        let motion = compound.getTagList("Motion", 6);
        let rotation = compound.getTagList("Rotation", 5);
        if pos.tagCount() >= 3 {
            self.entity.posX = pos.getDoubleAt(0);
            self.entity.posY = pos.getDoubleAt(1);
            self.entity.posZ = pos.getDoubleAt(2);
        ...
        if motion.tagCount() >= 3 {
            self.entity.motionX = motion.getDoubleAt(0);
            self.entity.motionY = motion.getDoubleAt(1);
            self.entity.motionZ = motion.getDoubleAt(2);
            if self.entity.motionX.abs() > 10.0 {
                self.entity.motionX = 0.0;
            }
            if self.entity.motionY.abs() > 10.0 {
                self.entity.motionY = 0.0;
            }
            if self.entity.motionZ.abs() > 10.0 {
                self.entity.motionZ = 0.0;
            }
        }
        if rotation.tagCount() >= 2 {
            self.entity.rotationYaw = rotation.getFloatAt(0);
            self.entity.rotationPitch = rotation.getFloatAt(1);
            self.entity.prevRotationYaw = self.entity.rotationYaw;
            self.entity.prevRotationPitch = self.entity.rotationPitch;
        }
        self.entity.fallDistance = compound.getFloat("FallDistance");
        self.entity.fire = compound.getShort("Fire") as i32;
        self.entity.onGround = compound.getBoolean("OnGround");
        if compound.hasKey("Dimension") {
            self.dimension = compound.getInteger("Dimension");
        }
        self.entity
            .setPosition(self.entity.posX, self.entity.posY, self.entity.posZ);
        self.inventory
            .readFromNBT(&compound.getTagList("Inventory", 10));
        self.inventory.currentItem = compound.getInteger("SelectedItemSlot");
        self.capabilities.readCapabilitiesFromNBT(&compound);
        self.managedPosX = self.entity.posX;
        self.managedPosZ = self.entity.posZ;
    ...
    pub fn writeToNBT(&self) -> NBTTagCompound {
        let mut compound = self
            .persistedNbt
            .clone()
            .unwrap_or_else(NBTTagCompound::new);
        let mut pos = NBTTagList::new();
        pos.appendTag(NBTBase::Double(self.entity.posX));
        pos.appendTag(NBTBase::Double(self.entity.posY));
        pos.appendTag(NBTBase::Double(self.entity.posZ));
        compound.setTagList("Pos", pos);
        let mut motion = NBTTagList::new();
        motion.appendTag(NBTBase::Double(self.entity.motionX));
        motion.appendTag(NBTBase::Double(self.entity.motionY));
        motion.appendTag(NBTBase::Double(self.entity.motionZ));
        compound.setTagList("Motion", motion);
        let mut rotation = NBTTagList::new();
        rotation.appendTag(NBTBase::Float(self.entity.rotationYaw));
        rotation.appendTag(NBTBase::Float(self.entity.rotationPitch));
        compound.setTagList("Rotation", rotation);
        compound.setFloat("FallDistance", self.entity.fallDistance);
        compound.setShort("Fire", self.entity.fire as i16);
        if !compound.hasKey("Air") {
            compound.setShort("Air", 300);
        }
        compound.setBoolean("OnGround", self.entity.onGround);
        compound.setInteger("Dimension", self.dimension);
        if !compound.hasKey("Invulnerable") {
            compound.setBoolean("Invulnerable", false);
        }
        if !compound.hasKey("PortalCooldown") {
            compound.setInteger("PortalCooldown", 0);
        }
        if let Some(uuid) = self.profile.getId() {
            compound.setUniqueId("UUID", uuid);
        }
        compound.setInteger("DataVersion", 1343);
        compound.setTagList("Inventory", self.inventory.writeToNBT(NBTTagList::new()));
        compound.setInteger("SelectedItemSlot", self.inventory.currentItem);
        if !compound.hasKey("Sleeping") {
            compound.setBoolean("Sleeping", false);
        }
        if !compound.hasKey("SleepTimer") {
            compound.setShort("SleepTimer", 0);
        }
        if !compound.hasKey("XpP") {
            compound.setFloat("XpP", 0.0);
        }
        if !compound.hasKey("XpLevel") {
            compound.setInteger("XpLevel", 0);
        }
        if !compound.hasKey("XpTotal") {
            compound.setInteger("XpTotal", 0);
        }
        if !compound.hasKey("XpSeed") {
            compound.setInteger("XpSeed", 0);
        }
        if !compound.hasKey("Score") {
            compound.setInteger("Score", 0);
        }
