...
impl PlayerList {
    ...
    pub fn newIntegrated() -> Self {
        Self {
            maxPlayers: 8,
            viewDistance: 10,
            playerChunkMap: PlayerChunkMap::new(10),
            hostPlayerData: None,
        }
    }
    pub const fn getMaxPlayers(&self) -> u8 {
        self.maxPlayers
    }
    pub const fn getViewDistance(&self) -> i32 {
        self.viewDistance
    }
    pub fn createPlayerForUser(
        &self,
        world: &mut WorldServer,
        profile: GameProfile,
    ) -> Result<EntityPlayerMP, String> {
        let gameType = world.worldInfo.getGameType();
        EntityPlayerMP::new(world, profile, gameType)
    }
    pub fn initializeConnectionToPlayer(
        &mut self,
        network: &mut NetworkManager,
        world: &mut WorldServer,
        profile: GameProfile,
        serverOwner: Option<&str>,
    ) -> Result<(EntityPlayerMP, NetHandlerPlayServer), String> {
        ...
        if host {
            if let Some(tag) = world.worldInfo.getPlayerNBTTagCompound().cloned() {
                player.readFromNBT(tag);
            } else {
                world
                    .saveHandler()
                    .base()
                    .readPlayerData(&mut player)
        ...
        if player.dimension != world.provider.getDimension() {
            return Err(format!(
                "saved player dimension {} is not loaded by this IntegratedServer tranche",
                player.dimension
            ));
        }
        ...
        network
            .sendPacket(
                &SPacketJoinGame::new(
                    ...
                    self.maxPlayers,
                    info.getTerrainType(),
                    info.getGameRulesInstance().getBoolean("reducedDebugInfo"),
                )
                .writePacketData()
                .map_err(|e| e.to_string())?,
            )
        ...
        write_string("vanilla", 32767, &mut brand).map_err(|e| e.to_string())?;
        network
            .sendPacket(
                &SPacketCustomPayload::new("MC|Brand", brand)
                    .writePacketData()
                    .map_err(|e| e.to_string())?,
            )
            .map_err(|e| e.to_string())?;
        network
            .sendPacket(
                &SPacketServerDifficulty::new(info.getDifficulty(), info.isDifficultyLocked())
                    .writePacketData(),
            )
            .map_err(|e| e.to_string())?;
        network
            .sendPacket(&SPacketPlayerAbilities::new(&player.capabilities).writePacketData())
            .map_err(|e| e.to_string())?;
        network
            .sendPacket(&SPacketHeldItemChange::new(player.inventory.currentItem).writePacketData())
            .map_err(|e| e.to_string())?;
        ...
        network
            .sendPacket(
                &SPacketTimeUpdate::new(
                    info.getWorldTotalTime(),
                    info.getWorldTime(),
                    info.getGameRulesInstance().getBoolean("doDaylightCycle"),
                )
                .writePacketData(),
            )
            .map_err(|e| e.to_string())?;
        network
            .sendPacket(&SPacketSpawnPosition::new(world.getSpawnPoint()).writePacketData())
            .map_err(|e| e.to_string())?;
        Ok((player, play))
    }
    /// MCP PlayerList / IntegratedPlayerList player-data write ownership.
    pub fn writePlayerData(
        &mut self,
        world: &mut WorldServer,
        ...
        world
            ...
            .map_err(|e| e.to_string())?;
        if serverOwner.is_some_and(|owner| player.getName() == owner) {
            self.hostPlayerData = Some(tag.clone());
            world.worldInfo.setPlayerNBTTagCompound(Some(tag));
        }
        Ok(())
    }
    pub fn getHostPlayerData(&self) -> Option<&NBTTagCompound> {
        self.hostPlayerData.as_ref()
    }

    pub fn tickPlayerChunks(
        &mut self,
        world: &mut WorldServer,
        network: &mut NetworkManager,
        player: &EntityPlayerMP,
    ) -> Result<usize, String> {
        self.playerChunkMap.tickPlayer(world, network, player)
    }
    pub fn updateMovingPlayer(
        &mut self,
        network: &mut NetworkManager,
        player: &mut EntityPlayerMP,
    ) -> Result<(), String> {
        for (x, z) in self.playerChunkMap.updateMovingPlayer(player) {
            network
                .sendPacket(&SPacketUnloadChunk::new(x, z).writePacketData())
                .map_err(|e| e.to_string())?;
        }
        Ok(())
    }
