...
impl PlayerInteractionManager {
    pub const fn new(gameType: GameType) -> Self { Self { gameType } }
    pub const fn getGameType(&self) -> GameType { self.gameType }
    pub const fn isCreative(&self) -> bool { self.gameType.isCreative() }
    pub fn setGameType(&mut self, gameType: GameType) { self.gameType = gameType; }
    ...
    pub fn onBlockClicked(
        ...
        if !self.isCreative() || !player.capabilities.allowEdit { return Ok(false); }
        if !(0..256).contains(&pos.y) { return Ok(false); }
        let current = world.getBlockStateAt(pos)?;
        if current.isAir() || current.getBlock().getBlockHardness() < 0.0 { return Ok(false); }
    ...
    pub fn processRightClickBlock(
        ...
        let _ = (hit_x, hit_z);
        if !(0..256).contains(&pos.y) || !player.capabilities.allowEdit { return Ok(EnumActionResult::Fail); }
        ...
        if held.isEmpty() || !ItemBlock::isItemBlock(&held) { return Ok(EnumActionResult::Pass); }
        ...
        let target = if clicked.getBlockId() == 78 || crate::net::minecraft::item::ItemBlock::isReplaceableState(clicked) {
            pos
        } else {
            pos.offset(side, 1)
        };
        if (0..256).contains(&target.y) { let _ = world.getBlockStateAt(target)?; }
        let Some(preview) = ItemBlock::serverPlacementState(
            world, pos, side, hit_y, &held, player.entity.rotationYaw, None,
        ) else {
            Self::sendBlockChange(network, world, pos)?;
            Self::sendBlockChange(network, world, pos.offset(side, 1))?;
    ...
    pub fn sendBlockChange(network: &mut NetworkManager, world: &mut WorldServer, pos: BlockPos) -> Result<(), String> {
        let state = world.getBlockStateAt(pos)?;
        network.sendPacket(&SPacketBlockChange::new(pos, state).writePacketData()).map_err(|e| e.to_string())
    }
}
