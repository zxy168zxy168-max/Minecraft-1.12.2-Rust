...
impl NBTTagList {
    ...
    pub fn appendTag(&mut self, nbt: NBTBase) {
        if nbt.getId() == TAG_END {
            return;
        }
        if self.tagType == TAG_END {
            self.tagType = nbt.getId();
        }
        if self.tagType == nbt.getId() {
            self.tagList.push(nbt);
        }
    }

    pub fn set(&mut self, index: usize, nbt: NBTBase) {
        if nbt.getId() == TAG_END || index >= self.tagList.len() {
            return;
        }
        if self.tagType == TAG_END {
            self.tagType = nbt.getId();
        ...
        if self.tagType == nbt.getId() {
            self.tagList[index] = nbt;
        }
    }

    pub fn removeTag(&mut self, index: usize) -> Option<NBTBase> {
        if index >= self.tagList.len() {
            return None;
        }
        let removed = self.tagList.remove(index);
        if self.tagList.is_empty() {
            self.tagType = TAG_END;
        }
        Some(removed)
    }

    pub fn hasNoTags(&self) -> bool {
        self.tagList.is_empty()
    }
    pub fn tagCount(&self) -> usize {
        self.tagList.len()
    }
    pub const fn getTagType(&self) -> u8 {
        self.tagType
    }
    pub fn tags(&self) -> &[NBTBase] {
        &self.tagList
    }
    ...
    pub(crate) fn write<W: Write>(&self, output: &mut W) -> io::Result<()> {
        ...
        output
            .write_i32::<BigEndian>(i32::try_from(self.tagList.len()).map_err(|_| {
                io::Error::new(io::ErrorKind::InvalidInput, "NBT list too large")
            })?)?;
    ...
    pub(crate) fn read<R: Read>(input: &mut R, depth: usize) -> io::Result<Self> {
        ...
        if length < 0 {
            return Err(io::Error::new(
                ...
                "Negative NBT list length",
            ));
        }
        if tagType == TAG_END && length > 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
        ...
        for _ in 0..length {
            tagList.push(NBTBase::readPayload(tagType, input, depth + 1)?);
        }
