...
impl ItemStack {
    ...
    pub fn fromNBT(nbt: &NBTTagCompound) -> Self {
        let Some(itemId) = itemIdByNameOrId(&nbt.getString("id")) else {
            return Self::EMPTY;
        };
        if itemId == 0 {
            return Self::EMPTY;
        ...
        let tagCompound = nbt
            .hasKeyWithType("tag", TAG_COMPOUND)
            .then(|| nbt.getCompoundTag("tag"));
        Self {
            itemId,
            count: count as u8,
            itemDamage,
    ...
    pub fn writeToNBT(&self, nbt: &mut NBTTagCompound) {
        let registry = if self.isEmpty() {
            "minecraft:air"
        } else {
            itemDefinition(self.itemId).registryName
        };
        nbt.setString("id", registry);
        nbt.setByte("Count", self.count as i8);
        nbt.setShort("Damage", self.itemDamage);
        if let Some(tag) = &self.tagCompound {
            nbt.setCompoundTag("tag", tag.clone());
        }
    }
    ...
    pub const fn getCount(&self) -> i32 {
        self.count as i32
    }
    pub fn setCount(&mut self, count: i32) {
        if count <= 0 {
            *self = Self::EMPTY;
            ...
            self.count = count.min(u8::MAX as i32) as u8;
        }
    }
    pub fn shrink(&mut self, amount: i32) {
        self.setCount(self.getCount() - amount.max(0));
    }
    ...
    pub fn getMaxStackSize(&self) -> i32 {
        if self.isEmpty() {
            64
        } else {
            Item::getItemStackLimit(self.itemId)
        }
    }
    pub fn getMaxDamage(&self) -> i32 {
        if self.isEmpty() {
            0
        } else {
            Item::getMaxDamage(self.itemId)
        }
    }
    pub fn getItemUseAction(&self) -> EnumAction {
        if self.isEmpty() {
            EnumAction::None
        } else {
            Item::getItemUseAction(self.itemId)
        }
    }
    pub fn getMaxItemUseDuration(&self) -> i32 {
        if self.isEmpty() {
            0
        } else {
            Item::getMaxItemUseDuration(self.itemId)
        }
    }
    pub fn getHasSubtypes(&self) -> bool {
        !self.isEmpty() && Item::getHasSubtypes(self.itemId)
    }
    pub fn isFood(&self) -> bool {
        !self.isEmpty() && Item::isFood(self.itemId)
    }
    pub fn isAlwaysEdible(&self) -> bool {
        !self.isEmpty() && Item::isAlwaysEdible(self.itemId)
    }
    pub fn isItemDamaged(&self) -> bool {
        self.isItemStackDamageable() && self.itemDamage > 0
    }
    pub fn showDurabilityBar(&self) -> bool {
        self.isItemDamaged()
    }
    pub fn getDurabilityForDisplay(&self) -> f64 {
        ...
        if maximum <= 0 {
            ...
            self.itemDamage.max(0) as f64 / maximum as f64
        }
    }
    /// MCP `ItemStack.hasDisplayName`: `display.Name` is a string tag.
    pub fn hasDisplayName(&self) -> bool {
        let Some(tag) = &self.tagCompound else {
            return false;
        };
        if !tag.hasKeyWithType("display", crate::net::minecraft::nbt::NBTBase::TAG_COMPOUND) {
            return false;
        }
        tag.getCompoundTag("display")
            .hasKeyWithType("Name", crate::net::minecraft::nbt::NBTBase::TAG_STRING)
    }

    pub fn isItemEnchanted(&self) -> bool {
        self.tagCompound
            .as_ref()
            .is_some_and(|tag| !tag.getTagList("ench", TAG_COMPOUND).hasNoTags())
    }
    pub fn hasEffect(&self) -> bool {
        if self.isItemEnchanted() {
            return true;
        }
        if matches!(self.itemId, 384 | 387 | 399 | 403 | 426) {
            return true;
        }
        if self.itemId == 322 && self.itemDamage > 0 {
            return true;
        }
        if matches!(self.itemId, 373 | 438 | 441) {
            ...
            if !tag
                .getTagList("CustomPotionEffects", TAG_COMPOUND)
                .hasNoTags()
            {
                return true;
            }
    ...
    pub fn getStrVsBlock(&self, state: IBlockState) -> f32 {
        if self.isEmpty() {
            ...
            Item::getDestroySpeed(self.itemId, state.getBlockId())
        }
    }
    ...
    pub fn canPlaceOn(&self, block: Block) -> bool {
        ...
        for index in 0..list.tagCount() {
            if Block::getBlockFromName(&list.getStringTagAt(index))
    ...
    pub const fn canEditBlocks(&self) -> bool {
        false
    }
    ...
    pub fn isItemStackDamageable(&self) -> bool {
        !self.isEmpty()
            ...
            && !self
                .tagCompound
                .as_ref()
                .is_some_and(|tag| tag.getBoolean("Unbreakable"))
    }
...
mod tests {
    ...
    fn nbt_round_trip_uses_registry_name_count_damage_and_tag() {
        let mut custom = NBTTagCompound::new();
        custom.setString("display-test", "yes");
        let original = ItemStack {
            itemId: 57,
            count: 23,
            itemDamage: 2,
            tagCompound: Some(custom),
        };
        let mut nbt = NBTTagCompound::new();
        original.writeToNBT(&mut nbt);
        assert_eq!(nbt.getString("id"), "minecraft:diamond_block");
        assert_eq!(nbt.getByte("Count"), 23);
        assert_eq!(nbt.getShort("Damage"), 2);
    ...
    fn can_place_on_reads_vanilla_adventure_tag() {
        ...
        list.appendTag(crate::net::minecraft::nbt::NBTBase::NBTBase::String(
    ...
    fn damageability_uses_the_full_registry_and_unbreakable_tag() {
        ...
        let protected = ItemStack {
            tagCompound: Some(unbreakable),
            ..bow
        };
