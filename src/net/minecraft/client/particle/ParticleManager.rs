...
use crate::net::minecraft::client::particle::ParticleDigging::{
    ParticleActualModel, ParticleDigging, ParticleDiggingRenderState,
};
...
impl ParticleManager {
    ...
    pub fn updateEffects(&mut self, world: &WorldClient) {
        for particle in &mut self.blockParticles {
            particle.onUpdate(world);
        }
        self.blockParticles.retain(|particle| !particle.isExpired());
        for particle in &mut self.miscParticles {
            particle.onUpdate(world);
        }
        self.miscParticles.retain(|particle| !particle.isExpired());
    }

    pub fn clearEffects(&mut self) {
        self.blockParticles.clear();
        self.miscParticles.clear();
    }

    fn addBlockEffect(&mut self, particle: ParticleDigging) {
        if self.blockParticles.len() >= 16_384 {
            self.blockParticles.remove(0);
        }
    ...
    fn addMiscEffect(&mut self, particle: ManagedParticle) {
        ...
        if self
            .miscParticles
            .iter()
            .filter(|entry| entry.isTransparent() == transparent)
            .count()
            >= 16_384
        {
            if let Some(index) = self
                .miscParticles
                .iter()
                .position(|entry| entry.isTransparent() == transparent)
            {
    ...
    pub fn spawnEffectParticle(&mut self, request: ParticleSpawnRequest) {
        ...
        let [xs, ys, zs] = request.speed;
        let particle =
            match request.particleType {
                EnumParticleTypes::WaterBubble => Some(ManagedParticle::Bubble(
                    ParticleBubble::new(x, y, z, xs, ys, zs, &mut self.random),
                )),
                EnumParticleTypes::SmokeNormal => Some(ManagedParticle::Smoke(
                    ParticleSmokeNormal::new(x, y, z, xs, ys, zs, 1.0, &mut self.random),
                )),
                // MCP ParticleSmokeLarge.Factory -> ParticleSmokeNormal(..., 2.5F).
                EnumParticleTypes::SmokeLarge => Some(ManagedParticle::Smoke(
                    ParticleSmokeNormal::new(x, y, z, xs, ys, zs, 2.5, &mut self.random),
                )),
                EnumParticleTypes::DragonBreath => Some(ManagedParticle::DragonBreath(
                    ParticleDragonBreath::new(x, y, z, xs, ys, zs, &mut self.random),
                )),
                EnumParticleTypes::EndRod => Some(ManagedParticle::EndRod(ParticleEndRod::new(
                    x,
                    y,
                    z,
                    xs,
                ...
                | EnumParticleTypes::SpellInstant
                | EnumParticleTypes::SpellMob
                | EnumParticleTypes::SpellMobAmbient
                | EnumParticleTypes::SpellWitch => Some(ManagedParticle::Spell(
                    ParticleSpell::new(request.particleType, x, y, z, xs, ys, zs, &mut self.random),
                )),
                EnumParticleTypes::Totem => Some(ManagedParticle::Totem(ParticleTotem::new(
                    x,
                    y,
                    z,
                    xs,
    ...
    pub fn spawnEffects(
        ...
        for request in requests {
            ...
            if request.alwaysRender
                && effectiveSetting == 2
                && self.visibilityRandom.next_i32_bound(10) == 0
            {
                effectiveSetting = 1;
            }
    ...
    pub fn addBlockDestroyEffects(
        ...
        for i in 0..4 {
            for j in 0..4 {
                for k in 0..4 {
                    let d0 = (i as f64 + 0.5) / 4.0;
                    let d1 = (j as f64 + 0.5) / 4.0;
                    let d2 = (k as f64 + 0.5) / 4.0;
                    let particle = ParticleDigging::new(
                        pos.x as f64 + d0,
                        pos.y as f64 + d1,
                        pos.z as f64 + d2,
                        d0 - 0.5,
    ...
    pub fn addBlockHitEffects(&mut self, world: &WorldClient, pos: BlockPos, side: EnumFacing) {
        ...
        if matches!(
            state.getBlockId(),
            0 | 36 | 63 | 68 | 119 | 144 | 166 | 176 | 177 | 209 | 217
        ) {
            return;
        }
        let Some(bounds) = world.getSelectedBoundingBox(pos) else {
            return;
        };
        let inset = 0.10000000149011612_f64;
        let random_axis = |random: &mut StdRng, min: f64, max: f64| {
            let span = max - min - inset * 2.0;
            min + inset + random.gen::<f64>() * span
        };
        ...
        match side {
            EnumFacing::Down => y = bounds.min_y - inset,
            EnumFacing::Up => y = bounds.max_y + inset,
            EnumFacing::North => z = bounds.min_z - inset,
            EnumFacing::South => z = bounds.max_z + inset,
            EnumFacing::West => x = bounds.min_x - inset,
            EnumFacing::East => x = bounds.max_x + inset,
        }
        let particle = ParticleDigging::new(x, y, z, 0.0, 0.0, 0.0, state, &mut self.random)
            .setBlockPos(pos, &self.blockColors, world)
            .multiplyVelocity(0.2)
            .multipleParticleScaleBy(0.6);
        self.addBlockEffect(particle);
    }

    pub fn renderStates(&self) -> Vec<ParticleDiggingRenderState> {
        self.blockParticles
            .iter()
            .map(ParticleDigging::renderState)
            .collect()
    }
    pub fn miscRenderStates(&self, partialTicks: f32) -> Vec<ParticleRenderState> {
        self.miscParticles
            .iter()
            .map(|particle| particle.renderState(partialTicks))
            .collect()
    }
    pub fn getStatistics(&self) -> String {
        (self.blockParticles.len() + self.miscParticles.len()).to_string()
    }
}
