...
pub enum GuiWorldSelectionAction {
    Select,
    Create,
    Edit,
    Delete,
...
pub struct GuiWorldSelectionInteraction {
    pub action: GuiWorldSelectionAction,
...
impl GuiWorldSelection {
    ...
    pub fn initGui(&mut self, width: i32, height: i32, locale: &Locale) {
        ...
        let mut select = GuiButton::newWithSize(
            1,
            width / 2 - 154,
            height - 52,
            150,
            20,
            locale.translate_key("selectWorld.select"),
        );
        ...
        for (id, x, key) in [
            ...
            let mut button =
                GuiButton::newWithSize(id, x, height - 28, 72, 20, locale.translate_key(key));
            if id != 0 {
    ...
    pub fn refreshList(&mut self) {
        match AnvilSaveConverter::new(&self.savesDirectory).getSaveList() {
            Ok(worlds) => {
                ...
                if self
                    .selected
                    .is_some_and(|index| index >= self.worlds.len())
                {
                    self.selected = None;
                }
            }
    ...
    fn listBottom(&self) -> i32 {
        self.GuiScreen.height - 64
    }
    fn maxScroll(&self) -> i32 {
        (self.worlds.len() as i32 * 36 - (self.listBottom() - self.listTop())).max(0)
    }
    ...
    fn syncButtons(&mut self) {
        ...
        for button in &mut self.GuiScreen.buttonList {
            if matches!(button.id, 1 | 2 | 4 | 5) {
                button.enabled = selected;
            }
        }
    }

    pub fn selectedWorld(&self) -> Option<&WorldSummary> {
    ...
    pub fn drawScreen(
        ...
        for (index, world) in self.worlds.iter().enumerate() {
            ...
            let display = if world.getDisplayName().is_empty() {
                format!(
                    "{} {}",
                    locale.translate_key("selectWorld.world"),
                    index + 1
                )
            } else {
                world.getDisplayName().to_owned()
            };
            ...
            let mut mode = if world.isHardcoreModeEnabled() {
                ...
                locale
                    .translate_key(&format!("gameMode.{}", world.getEnumGameType().getName()))
                    .to_owned()
            };
            if world.getCheatsEnabled() {
                mode.push_str(&format!(", {}", locale.translate_key("selectWorld.cheats")));
            }
            mode.push_str(&format!(
                ", {} {}",
                locale.translate_key("selectWorld.version"),
                world.getVersionName()
            ));
            self.GuiScreen
                .Gui
                .drawString(font, drawList, &display, left + 35, y + 1, 0x00FF_FFFF);
    ...
    pub fn mouseClicked(
        ...
        mouseY: i32,
        mouseButton: i32,
    ) -> Option<GuiWorldSelectionInteraction> {
        if mouseButton != 0 {
            return None;
        }
        let left = self.GuiScreen.width / 2 - 110;
        ...
        self.GuiScreen.buttonList.iter().find_map(|button| {
            ...
            Some(GuiWorldSelectionInteraction {
                action,
