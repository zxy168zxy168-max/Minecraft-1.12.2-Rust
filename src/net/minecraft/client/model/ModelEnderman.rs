...
mod tests {
    ...
    #[test]
    fn attacking_lowers_head_five_model_pixels() {
        assert_eq!(
            ModelEnderman::pose(input(), false, false).head.pivot[1],
            -13.0
        );
        assert_eq!(
            ModelEnderman::pose(input(), false, true).head.pivot[1],
            -18.0
        );
    }
    #[test]
    fn carrying_forces_vanilla_arm_pose() {
