...
use crate::net::minecraft::network::PacketBuffer::{
    read_f32_be, read_f64_be, read_u8, read_var_i32, write_f32_be, write_f64_be, write_var_i32,
    CodecError,
};
...
pub struct SPacketPlayerPosLook {
    x: f64,
    y: f64,
    z: f64,
    yaw: f32,
...
impl SPacketPlayerPosLook {
    ...
    pub fn writePacketData(&self) -> RawPacket {
        ...
        payload.push(self.flags.bits());
        write_var_i32(self.teleportId, &mut payload);
        RawPacket::new(0x2F, payload)
    }
    pub fn readPacketData(packet: &RawPacket) -> Result<Self, CodecError> {
        ...
        Ok(Self {
            ...
            flags: EnumFlags::from_bits_truncate(read_u8(&mut input)?),
            teleportId: read_var_i32(&mut input)?,
        })
    }
    ...
    pub const fn getFlags(&self) -> EnumFlags {
        self.flags
    }
}
