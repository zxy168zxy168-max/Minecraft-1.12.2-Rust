...
use crate::net::minecraft::network::PacketBuffer::{
    read_bool, read_i32_be, read_nbt_compound, read_var_i32, write_bool, write_i32_be,
    write_nbt_compound, write_var_i32, CodecError,
};
...
impl SPacketChunkData {
    ...
    pub fn new(chunk: &Chunk, changedSectionFilter: i32, hasSkyLight: bool) -> Self {
        ...
        for (index, storage) in chunk.getBlockStorageArray().iter().enumerate() {
            let Some(storage) = storage.as_deref() else {
                continue;
            };
            if (!loadChunk || !storage.isEmpty()) && (changedSectionFilter & (1_i32 << index)) != 0
            {
                ...
                if hasSkyLight {
                    if let Some(skylight) = storage.getSkylightArray() {
                        buffer.extend_from_slice(skylight.getData());
                    } else {
                        buffer.extend(std::iter::repeat(0_u8).take(2048));
                    }
                }
            }
        }
        if loadChunk {
            buffer.extend_from_slice(chunk.getBiomeArray());
        }
        ...
        for (pos, tag) in chunk.getTileEntityMapData() {
            ...
            if loadChunk
                || (section >= 0
                    && section < 16
                    && (changedSectionFilter & (1_i32 << section)) != 0)
            {
    ...
    pub fn writePacketData(&self) -> Result<RawPacket, CodecError> {
        ...
        write_i32_be(self.chunkX, &mut payload);
        write_i32_be(self.chunkZ, &mut payload);
        write_bool(self.loadChunk, &mut payload);
        write_var_i32(self.availableSections, &mut payload);
        write_var_i32(self.buffer.len() as i32, &mut payload);
        payload.extend_from_slice(&self.buffer);
        write_var_i32(self.tileEntityTags.len() as i32, &mut payload);
        for tag in &self.tileEntityTags {
            write_nbt_compound(Some(tag), &mut payload)?;
        }
    ...
    pub fn readPacketData(packet: &RawPacket) -> Result<Self, CodecError> {
        ...
        let (buffer, remainder) = input.split_at(length as usize);
        input = remainder;
        ...
        for _ in 0..count {
            if let Some(tag) = read_nbt_compound(&mut input)? {
                tileEntityTags.push(tag);
            }
        }
        Ok(Self {
            chunkX,
    ...
    pub const fn getExtractedSize(&self) -> i32 {
        self.availableSections
    }
    pub const fn doChunkLoad(&self) -> bool {
        self.loadChunk
    }
    pub fn getReadBuffer(&self) -> &[u8] {
        &self.buffer
    }
    pub fn getTileEntityTags(&self) -> &[NBTTagCompound] {
        &self.tileEntityTags
    }
}
