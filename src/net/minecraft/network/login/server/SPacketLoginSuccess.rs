...
pub struct SPacketLoginSuccess {
    profile: GameProfile,
}
impl SPacketLoginSuccess {
    ...
    pub fn readPacketData(packet: &RawPacket) -> Result<Self, SPacketLoginSuccessError> {
        ...
        if !input.is_empty() {
            return Err(SPacketLoginSuccessError::Codec(CodecError::InvalidData(
                format!("{} unread login-success bytes", input.len()),
            )));
        }
        let id = Uuid::parse_str(&uuidText)
            .map_err(|error| SPacketLoginSuccessError::InvalidUuid(error.to_string()))?;
        Ok(Self {
            profile: GameProfile::new(Some(id), name),
        })
    }
    pub fn writePacketData(&self) -> Result<RawPacket, SPacketLoginSuccessError> {
        let id = self
            .profile
            .getId()
            .ok_or(SPacketLoginSuccessError::MissingUuid)?;
...
pub enum SPacketLoginSuccessError {
    ...
    #[error("invalid login UUID: {0}")]
    InvalidUuid(String),
    #[error("login success profile has no UUID")]
    MissingUuid,
}
