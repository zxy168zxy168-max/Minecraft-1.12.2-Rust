...
impl SecretKey {
    pub const fn fromBytes(bytes: [u8; 16]) -> Self {
        Self(bytes)
    }
    pub const fn getEncoded(&self) -> &[u8; 16] {
        &self.0
    }
}
...
impl KeyPair {
    pub fn getPrivate(&self) -> &RsaPrivateKey {
        &self.privateKey
    }
    pub fn getPublic(&self) -> &RsaPublicKey {
        &self.publicKey
    }
}
...
impl NetCipher {
    pub fn new(secretKey: &SecretKey, encrypting: bool) -> Self {
        ...
        Self {
            cipher,
            shiftRegister: *secretKey.getEncoded(),
            encrypting,
        }
    }
...
mod tests {
    ...
    fn cfb8_encrypt_decrypt_roundtrip_is_stateful() {
        ...
        assert_eq!(
            hex(&encrypted),
            "3fda07bf434480a92711e2066f941fa508fa5ba78907758aa93087b4a4"
        );
