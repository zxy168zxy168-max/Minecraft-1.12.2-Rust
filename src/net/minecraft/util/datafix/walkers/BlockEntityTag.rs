...
impl BlockEntityTag {
    fn getBlockEntityID(blockID: i32, item: &str) -> Option<&'static str> {
        ...
        if blockID < 515 { match item.as_str() {
...
impl IDataWalker for BlockEntityTag {
    fn process(&self, fixer:&dyn IDataFixer, mut compound:NBTTagCompound, versionIn:i32)->NBTTagCompound {
        ...
        if tag.hasKeyWithType("BlockEntityTag",TAG_COMPOUND) {
            ...
            if removeId { blockEntity.removeTag("id"); }
...
#[cfg(test)] mod tests { use super::*; #[test] fn source_tables_keep_pre_and_post_515_ids() { assert_eq!(BlockEntityTag::getBlockEntityID(500,"furnace"),Some("Furnace")); assert_eq!(BlockEntityTag::getBlockEntityID(515,"furnace"),Some("minecraft:furnace")); assert_eq!(BlockEntityTag::getBlockEntityID(515,"bed"),Some("minecraft:bed")); } }
