...
fn processInventory(
    ...
    if compound.hasKeyWithType(key, TAG_LIST) {
        ...
        for i in 0..list.tagCount() {
            let fixed =
                fixer.processVersioned(FixTypes::ItemInstance, list.getCompoundTagAt(i), versionIn);
...
impl IDataWalker for VillagerTradeDataWalker {
    fn process(
        ...
        if ResourceLocation::parse(compound.getString("id")) != ResourceLocation::parse("villager")
            || !compound.hasKeyWithType("Offers", TAG_COMPOUND)
        {
            return compound;
        }
...
impl IDataWalker for MobSpawnerDataWalker {
    fn process(
        ...
        if ResourceLocation::parse(compound.getString("id"))
            != ResourceLocation::parse("mob_spawner")
        {
            return compound;
        }
        if compound.hasKeyWithType("SpawnPotentials", TAG_LIST) {
            ...
            for i in 0..list.tagCount() {
                ...
                let fixed = fixer.processVersioned(
                    FixTypes::Entity,
                    potential.getCompoundTag("Entity"),
                    versionIn,
                );
        ...
        let fixed = fixer.processVersioned(
            FixTypes::Entity,
            compound.getCompoundTag("SpawnData"),
            versionIn,
        );
...
impl IDataWalker for MinecartSpawnerDataWalker {
    fn process(
        ...
        if ResourceLocation::parse(&original) != ResourceLocation::parse("spawner_minecart") {
            return compound;
        }
...
impl IDataWalker for MinecartCommandBlockDataWalker {
    fn process(
        ...
        if ResourceLocation::parse(&original) != ResourceLocation::parse("command_block") {
            return compound;
        }
