...
#[derive(Debug, Clone)]
pub struct Filtered { key: ResourceLocation }
impl Filtered {
    pub fn new(registryName: &str) -> Self { Self { key: ResourceLocation::parse(registryName) } }
    pub fn matches(&self, compound: &NBTTagCompound) -> bool {
        ResourceLocation::parse(compound.getString("id")) == self.key
    }
    pub fn key(&self) -> &ResourceLocation { &self.key }
    pub fn processIf<F>(&self, fixer: &dyn IDataFixer, compound: NBTTagCompound, versionIn: i32, process: F) -> NBTTagCompound
    where F: FnOnce(&dyn IDataFixer, NBTTagCompound, i32) -> NBTTagCompound {
        if self.matches(&compound) { process(fixer, compound, versionIn) } else { compound }
    }
}
