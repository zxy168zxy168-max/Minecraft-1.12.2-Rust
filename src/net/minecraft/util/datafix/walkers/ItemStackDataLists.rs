...
pub struct ItemStackDataLists {
    filtered: Filtered,
    matchingTags: Vec<&'static str>,
}
impl ItemStackDataLists {
    pub fn new(registryName: &str, matchingTags: &[&'static str]) -> Self {
        Self {
            filtered: Filtered::new(registryName),
            matchingTags: matchingTags.to_vec(),
        }
    }
}
impl IDataWalker for ItemStackDataLists {
    fn process(
        ...
        self.filtered.processIf(
            fixer,
            compound,
            versionIn,
            |fixer, mut compound, versionIn| {
