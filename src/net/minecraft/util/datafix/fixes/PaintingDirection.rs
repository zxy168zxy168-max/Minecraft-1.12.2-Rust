...
impl IFixableData for PaintingDirection {
    ...
    fn fixTagCompound(&self, mut compound: NBTTagCompound) -> NBTTagCompound {
        ...
        let painting = id == "Painting";
        let frame = id == "ItemFrame";
        if (painting || frame) && !compound.hasKeyWithType("Facing", 99) {
            let horizontal = if compound.hasKeyWithType("Direction", 99) {
                ...
                if frame && compound.hasKeyWithType("ItemRotation", 99) {
                    compound.setByte(
                        "ItemRotation",
                        compound.getByte("ItemRotation").wrapping_mul(2),
                    );
                }
                h
            } else {
                let h = ((compound.getByte("Dir") as i32 % 4).abs()) as u8;
