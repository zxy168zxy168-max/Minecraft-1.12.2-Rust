...
impl IFixableData for BookPagesStrictJSON {
    ...
    fn fixTagCompound(&self, mut compound: NBTTagCompound) -> NBTTagCompound {
        if compound.getString("id") != "minecraft:written_book" {
            return compound;
        }
        ...
        if tag.hasKeyWithType("pages", TAG_LIST) {
            ...
            for index in 0..pages.tagCount() {
                let fixed =
                    SignStrictJSON::normalizeTextComponentJson(&pages.getStringTagAt(index));
...
mod tests {
    ...
    fn written_book_pages_use_same_strict_component_migration_as_signs() {
        let mut pages = NBTTagList::new();
        pages.appendTag(NBTBase::String("hello".into()));
        let mut tag = NBTTagCompound::new();
        tag.setTagList("pages", pages);
        let mut book = NBTTagCompound::new();
        book.setString("id", "minecraft:written_book");
        book.setCompoundTag("tag", tag);
        let fixed = BookPagesStrictJSON.fixTagCompound(book);
        assert_eq!(
            fixed
                .getCompoundTag("tag")
                .getTagList("pages", TAG_STRING)
                .getStringTagAt(0),
            r#"{"text":"hello"}"#
        );
    }
