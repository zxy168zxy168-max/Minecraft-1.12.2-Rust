...
impl IFixableData for BannerItemColor {
    ...
    fn fixTagCompound(&self, mut compound: NBTTagCompound) -> NBTTagCompound {
        if compound.getString("id") != "minecraft:banner" || !compound.hasKeyWithType("tag", 10) {
            return compound;
        }
        ...
        if !block_entity.hasKeyWithType("Base", 99) {
            return compound;
        }
        ...
        if tag.hasKeyWithType("display", 10) {
            ...
            if display.hasKeyWithType("Lore", 9) {
                ...
                if lore.tagCount() == 1 && lore.getStringTagAt(0) == "(+NBT)" {
                    return compound;
                }
            }
        }
