...
impl IFixableData for ShulkerBoxItemColor {
    fn getFixVersion(&self) -> i32 { 813 }
    fn fixTagCompound(&self, mut compound: NBTTagCompound) -> NBTTagCompound {
        if compound.getString("id") != "minecraft:shulker_box" || !compound.hasKeyWithType("tag", 10) { return compound; }
        ...
        if block_entity.getTagList("Items", 10).hasNoTags() { block_entity.removeTag("Items"); }
