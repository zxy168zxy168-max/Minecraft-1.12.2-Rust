...
pub const fn onBlockPlacedState(blockId:i32,facing:EnumFacing)->IBlockState {
    let axisBits=match facing.axis(){Axis::X=>4,Axis::Z=>8,Axis::Y=>0};
    IBlockState::fromGlobalStateId((blockId<<4)|axisBits)
}

#[cfg(test)]mod tests{use super::*;#[test]fn axis_bits_match_mcp(){assert_eq!(onBlockPlacedState(170,EnumFacing::Up).getMetadata(),0);assert_eq!(onBlockPlacedState(170,EnumFacing::East).getMetadata(),4);assert_eq!(onBlockPlacedState(170,EnumFacing::North).getMetadata(),8);}}
