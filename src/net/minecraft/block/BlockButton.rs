...
pub fn canPlaceBlock<A: IBlockAccess>(world: &A, pos: BlockPos, direction: EnumFacing) -> bool {
    ...
    let solid = support.getBlockFaceShape(world, supportPos, direction)
        == crate::net::minecraft::block::state::BlockFaceShape::BlockFaceShape::SOLID;
    let id = support.getBlockId();
    let blockedTop = matches!(
        id,
        18 | 161 | 96 | 167 | 138 | 118 | 20 | 89 | 79 | 169 | 95 | 219..=234
    );
    let blockedSide = blockedTop || matches!(id, 29 | 33 | 34);
    if direction == EnumFacing::Up {
        id == 154 || (!blockedTop && solid)
    } else {
        !blockedSide && solid
    }
}
...
pub fn onBlockPlacedState<A: IBlockAccess>(
    ...
    let facing = if canPlaceBlock(world, pos, facingIn) {
        facingIn
    } else {
        EnumFacing::Down
    };
    let meta = match facing {
        EnumFacing::Down => 0,
        EnumFacing::East => 1,
        EnumFacing::West => 2,
        EnumFacing::South => 3,
