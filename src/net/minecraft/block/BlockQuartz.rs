...
pub const fn isBlockQuartz(state: IBlockState) -> bool {
    state.getBlockId() == 155
}
...
mod tests {
    use super::*;
    #[test]
    fn line_quartz_rotates_from_face_axis() {
        assert_eq!(onBlockPlacedState(EnumFacing::Up, 2).getMetadata(), 2);
        assert_eq!(onBlockPlacedState(EnumFacing::East, 2).getMetadata(), 3);
        assert_eq!(onBlockPlacedState(EnumFacing::North, 2).getMetadata(), 4);
    }
}
