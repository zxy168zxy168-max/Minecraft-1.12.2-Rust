...
pub const fn isBlockAnvil(state: IBlockState) -> bool {
    state.getBlockId() == 145
}
...
pub fn onBlockPlacedState(placerYaw: f32, itemDamage: i16) -> IBlockState {
    ...
    let damage = if (0..=2).contains(&(itemDamage as i32)) {
        itemDamage as i32
    } else {
        0
    };
...
mod tests {
    use super::*;
    #[test]
    fn damage_and_rotated_facing_match_mcp() {
        let state = onBlockPlacedState(0.0, 2);
        assert_eq!(state.getMetadata() >> 2, 2);
        assert_eq!(state.getMetadata() & 3, 3);
        assert_eq!(onBlockPlacedState(0.0, 9).getMetadata() >> 2, 0);
    }
}
